#!/bin/bash
echo "MariaDB Backup Script Starts......"

input="/dbbackup-scripts/mariadb-list.txt"
while read -r line
do
          echo "--------------------------------------------------------"
		  IFS=":"
          ...
          echo $FILENAME
          echo "MariaDB Backup Generates File /tmp/$FILENAME.sql.gz"

		  service=${str[0]}
		  namespace=${str[1]}
		  export password=$(oc get secret -n $namespace $service -o jsonpath="{.data.mariadb-root-password}" | base64 -d)
		  ...
		  if [ "$?" -eq 0 ]; then
				...
				if [[ $FILESIZE -gt $SIZELIMIT ]]; then 
					...
					for file in /tmp/$FILENAME.sql.part*; do
						...
						echo "$uploadfile"
						echo "MariaDB Backup Dump Split /tmp/$uploadfile is being Uploaded into cos-4-db-backup COS and roks-dev-mysqldbbackup bucket"
						nodejs /dbbackup-scripts/cos-connect.js /tmp/$uploadfile $uploadfile roks-dev-mysqldbbackup
					done
				else 
					echo "MariaDB Backup Dump /tmp/$FILENAME.sql.gz is being Uploaded into cos-4-db-backup COS and roks-dev-mysqldbbackup bucket"
					nodejs /dbbackup-scripts/cos-connect.js /tmp/$FILENAME.sql.gz $FILENAME roks-dev-mysqldbbackup
				fi
		  else
...
nodejs /dbbackup-scripts/cos-connect.js /tmp/mariadb_backup_$(date +%F).log mariadb_backup_$(date +%F).log roks-dev-mysqldbbackup
#this part will iterate for as many MySQL instances in Cluster - end

echo "MariaDB Backup Script Ends......"